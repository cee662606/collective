...
% data into the Matlab environment in the form of Matlab arrays or matrices.  
% 
% Note: All Timestamps are in microseconds.
% Note: The size of the Data Points field below is based on the filetype in
% which the number of electrodes is equal to the size of the 3rd dimension
% of the matrix.
% 
% NumFields = 5;
% Field List
...
%     2. Extract Record Index Range = This will extract every record whose index is within a range specified by parameter 5.
%     3. Extract Record Index List = This will extract every record whose index in the file is the same index that is specified by parameter 5.
%     4. Extract Timestamp Range = This will extract every record whose timestamp is within a range specified by parameter 5.
%     5. Extract Timestamp List = This will extract every Record with the same timestamp that is specified by parameter 5.
%     
% Input Parameters
% 
%     Parameter1 - Input File Name and Path
%                  Requirements
%                  1. The file must be a Neuralynx SE(Single Electrode), ST(Stereotrode) or TT(Tetrode) data file.
%                  2. The filename must be a string and a row vector.
%                  Example:  
%                      Filename = 'C:\TestDirector\TestFile.nse'
%                  
...
% Output Parameters
%
% The 6 variables Nlx2MatSpike outputs are:
...
%         
%         Example: If we continue with the example from Parameter2 and we know that we want the Timestamp and Samples fields extracted, the output will be as follows:
% 
%         [TimestampVariable, SamplesVariable, HeaderVariable] = Function( Filename, FieldSelection, ExtractHeader, ExtractMode );
%         
...
%
%   Formatting numeric data display.
